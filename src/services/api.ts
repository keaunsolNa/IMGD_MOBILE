...
export const UserAPI = {
  findUserByToken: () => api.get(`/api/user`),
  findUserById: (userId: string) => api.get(`/api/user/${userId}`),
  updateUser: (userData: { userId: string; nickName: string }) => api.post(`/api/user`, userData),
  deleteUser: (userId: string) => api.delete(`/api/user`, { params: { userId } }),
  findFriendEachOther: (userId: string) => api.get(`/api/user/${userId}/friends`, { params: { option: 1 } }),
  findFriendWhoAddMeButImNot: (userId: string) => api.get(`/api/user/${userId}/friends`, { params: { option: 2 } }),
  findFriendWhoImAddButNot: (userId: string) => api.get(`/api/user/${userId}/friends`, { params: { option: 3 } }),
  findFriendWhoImAddButReject: (userId: string) => api.get(`/api/user/${userId}/friends`, { params: { option: 4 } }),
  findFriend: (userId: string) => api.get(`/api/user/${userId}/friends`, { params: { option: 5 } }),
  searchFriend: (userId: string) => api.get(`/api/user/relationship`, { params: { userId } }),
  findFriendEachOtherAndNotInGroup: (userId: string, groupId: number) => api.get(`/api/user/group`, { params: { userId, groupId } }),
  insertUserFriendTable: (userId: string, targetUserId: string, relationship: string = 'F') => api.post(`/api/userFriend?userId=${userId}&targetUserId=${targetUserId}&relationship=${relationship}`),
  deleteUserFriendTable: (userId: string, targetUserId: string) => api.delete(`/api/userFriend`, { params: { userId, targetUserId } }),
